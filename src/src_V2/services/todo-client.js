import { ReminderStatus, TaskPriority, TaskStatus } from '../modules/manager/todo/utils';
...
const TodoClient = {
  getTodos: async (filters) => {
    ...
    if (filters.statuses) {
      filters.statuses.split(',').forEach((status) =>
        adaptedFilters.push({
          fieldName: 'Status',
  ...
  updateTodo: (todo) => {
    return ApiClient.put('api/v1/ToDo', todoUpdateAdapterAPI(todo));
  },
  getTodoById: async (id) => {
    const { data } = await ApiClient.get(`api/v1/ToDo/${id}`);
    return todoAdapterClient(data);
  },
  deleteTodo: (id) => ApiClient.delete(`api/v1/ToDo/${id}`),

  duplicateTodo: (todo) => ApiClient.post('api/v1/ToDo', todoUpdateAdapterAPI(todo)),
};
